...
import {
  removeExpenseList,
  saveExpenseList,
} from '../../../redux/slices/expenseListSlice';
...
const Expense = ({navigation}) => {
  ...
  const filteredExpense = expenseList?.filter(t => {
  ...
  const getExpense = async () => {
    setError(null);
    try {
      const response = await getApi('expense_data', {
        uid: authUser?.uid,
      });
      // response && setExpenseList(response);
      if (response) {
        dispatch(saveExpenseList(response));
      }
    } catch (error) {
      setError(error);
    } finally {
