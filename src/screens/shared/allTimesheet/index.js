...
import {
  removeAllTimesheetList,
  saveAllTimesheetList,
} from '../../../redux/slices/allTimesheetListSlice';

const AllTimeSheet = ({navigation}) => {
  ...
  const filteredTimeSheet = allTimesheetList?.filter(t =>
  ...
  const filterDataByWeek = () => {
    ...
    const filteredData = allTimesheetList?.filter(item => {
  ...
  const getTimeSheet = async () => {
    setError(null);
    try {
      const response = await getApi('get/worker/timesheet', {
        uid: authUser?.uid,
      });
      if (response) {
        dispatch(saveAllTimesheetList(response));
      }
    } catch (error) {
      setError(error);
    } finally {
  ...
  useEffect(() => {
    ...
    return () => {
      setError(null);
      setLoading(false);
      dispatch(removeAllTimesheetList());
    };
  }, []);
